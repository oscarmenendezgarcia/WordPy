...
def create_visualization(text, thumbnails):
        ...
        image = Image.open(requests.get(thumbnails, stream=True).raw)
        image.thumbnail([540,360], Image.ANTIALIAS)
        
        mask = np.array(image)
        mask_transformed = np.ndarray((mask.shape[0],mask.shape[1]), np.int32)

        image_colors = ImageColorGenerator(mask_transformed)

        for i in range(len(mask_transformed)):
                mask_transformed[i] = list(map(transform_format, mask_transformed[i]))

        wordcloud = WordCloud(background_color="white", contour_width=3, contour_color='firebrick', mask=mask_transformed, stopwords=stop_words).generate(text)
...
thumbnails = download_audio(url)
text = transcribe_audio(path)
create_visualization(text, thumbnails)